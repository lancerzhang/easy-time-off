...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        boolean seedPods = podRepository.count() == 0;
        boolean seedTeams = teamRepository.count() == 0;
        boolean seedLeaves = leaveRepository.count() == 0;

        if (!seedUsers && !seedPods && !seedTeams && !seedLeaves) {
        ...
        // 2. Create Pods (Agile Pods stored separately)
        if (seedPods) {
            Pod pod1 = Pod.builder().id("pod1").name("Checkout Pod").memberIds(Set.of("u1", "u2", "u3")).build();
            Pod pod2 = Pod.builder().id("pod2").name("Inventory Pod").memberIds(Set.of("u4", "u5")).build();

            podRepository.saveAll(Arrays.asList(pod1, pod2));
        }

        // 3. Create Virtual Teams
        if (seedTeams) {
            Team vt1 = Team.builder()
                    .id("vt1")
                    .name("Backend Guild")
                    .type(Team.TeamType.VIRTUAL)
                    .memberIds(Set.of("u2", "u5"))
                    .createdBy("u1")
                    .build();

            teamRepository.saveAll(Arrays.asList(vt1));
        }
